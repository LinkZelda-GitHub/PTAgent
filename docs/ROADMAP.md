...
| `repository` | 账号/教师资料文件数据库 + 其余业务内存 Map | 替换为 MyBatis-Plus/JPA + MySQL |
...
1. 将写操作中的操作者 ID 收敛到服务端认证上下文，避免信任请求体身份字段。
2. 先为账号与教师领域实现 MySQL Repository，再逐步迁移需求、申请、订单、通知和审计数据。
3. 迁移 Spring Boot Controller，但保持现有 API 兼容。
4. 将当前迭代式密码哈希替换为 BCrypt 或 Argon2，并补充密码策略与重置流程。
...
- 已新增 `AccessGuard`、内存 `AuditLog`、`/api/audit-logs` 与前端“审计日志”页面，关键写操作开始具备角色校验和审计记录。

2026-06-27：

- 已新增 12 小时内存登录会话、Bearer Token API 门禁、会话恢复与主动退出，取消前端启动时自动登录。
- 已调整登录态布局：登录后隐藏右侧登录栏，左侧使用与业务功能对应的图标文字导航。
- 已增加认证会话生命周期测试和真实 HTTP 登录/退出链路验证；下一步继续把请求体操作者 ID 收敛到服务端认证上下文。
- 已实现完整教师信息注册、字段校验、重复账号/手机/邮箱检查和待审核启用流程。
- 已建立零依赖本地账号数据库，账号与教师资料可跨服务重启恢复；同时新增 MySQL 8 初始化迁移和数据库初始化脚本。
- 已增加注册持久化测试和 HTTP 验收链路；下一步把需求、申请、订单、通知和审计日志继续迁移到 MySQL Repository。
