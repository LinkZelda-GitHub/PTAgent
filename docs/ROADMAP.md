...
- 增加统一错误码，例如 `AUTH_VERIFICATION_CODE_INVALID`、`DEMAND_NOT_FOUND`、`APPLICATION_DUPLICATED`。
...
- 微信/QQ OAuth授权码必须由服务端换取用户身份，手机号验证码接入真实短信通道与风控。
- 后端强制角色权限，前端只做展示控制。
...
1. 先为账号与教师领域实现 MySQL Repository，再逐步迁移需求、申请、订单、通知和审计数据。
2. 迁移 Spring Boot Controller，但保持现有 API 兼容，并把轻量认证上下文替换为 Spring Security。
3. 接入微信开放平台、QQ互联和短信服务商，并补充登录限流与账号绑定/解绑流程。
4. 引入 Redis，持久化可撤销会话、验证码、限流状态和热点查询缓存。
...
- 已移除用户名密码登录，统一为微信、QQ、手机号验证码三选一登录。
- 已将本地账号数据库升级到 schema v2，并增加旧账号自动迁移与 MySQL V2 无密码迁移。
- 已重做登录/注册界面和演示身份入口；生产环境下一步接入真实 OAuth 与短信服务商。
- 已将写操作操作者 ID 收敛到 Bearer 会话认证上下文，客户端伪造身份字段会被服务端覆盖。
- 已按登录角色收敛教师申请、资料、简历、订单、授课记录和通知查询范围，并在请求日志中记录认证用户 ID。
- 已在开发备忘录集中登记微信、QQ、短信、地图、MySQL、Redis、对象存储、消息和监控等待接服务。
- 已形成 `0.9.0-rc1` 预集成候选版：增加请求上限、严格 JSON、同源 CORS、安全响应头、认证防刷和需求隐私脱敏。
- 已增加环境变量模板、构建产物清理、发布包启动脚本和自动测试打包脚本，交付 ZIP 排除数据与密钥。
- 已形成 `0.9.0-rc2`：新增 `/live`、`/ready`、`/dependencies` 探针，生产适配器接入前不会误报就绪。
- 已将 Request ID、来源 IP、User-Agent 和执行结果接入审计上下文，审计日志追加落盘并通过 SHA-256 哈希链检测篡改。
- 下一阶段不再扩展本地替代能力，集中接入 MySQL、Redis、正式 OAuth/短信和集中可观测平台。
