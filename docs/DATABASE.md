...
关键操作审计单独追加到 `data/ptagent-audit.jsonl`。每条 JSON 记录保存前一条哈希和自身 SHA-256 哈希，启动时会验证链顺序与内容完整性；这不替代生产数据库权限隔离、可信链头、WORM/集中日志存储与备份。
...
`V1__init.sql` 提供数据库基线，`V2__passwordless_login.sql` 将旧账号表迁移为无密码身份模型，`V3__audit_context_and_integrity.sql` 为审计表补充来源、结果、哈希链字段和查询索引。`user_account` 通过 `(login_method, login_id)` 唯一约束避免重复绑定。后续结构变更继续新增版本化迁移文件。
