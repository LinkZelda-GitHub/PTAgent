...
当前实现没有外部 jar、npm 包或 CDN 资源，适合在离线环境中运行和评审。运行时会自动创建 `data/ptagent-accounts.json`；该文件保存登录身份和教师资料，不保存密码或密码哈希。

`0.9.0-rc1` 可生成零依赖预集成交付 ZIP，但这不代表已经满足生产基础设施、外部认证、短信、数据库、对象存储和监控要求。
